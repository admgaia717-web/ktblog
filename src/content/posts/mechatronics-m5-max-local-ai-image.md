...
![M5 Max 三層構成アーキテクチャ図](/assets/rqgm83/svg-architecture.svg)
*図1: M5 Max 128GBの三層構成アーキテクチャ。Draw Thingsが高速レンダリング核、ComfyUIがワークフロー自動化、MFLUXが最新モデル実験場を担当する。*

---

🚀 **本記事のポイント：**
- M5 Max 128GBでは FLUX.2 dev 32B が実用的に動作
- Draw Things + ComfyUI + MFLUX の三層構成が最強
- ラフ生成〜2秒、本番品質〜30秒、全行程をノートPC一台で完結
...
> **ローカル作画環境は「贅沢」ではなく「投資」になる。以下のいずれかに当てはまるなら、検討に値する。**
...
そして、**M5 Max（40コアGPU・128GBユニファイドメモリ）** は、2026年夏現在、ノートPCで実現できる最強のローカル作画環境だ。

本稿では、このマシンで何ができて、どう構成すべきかを、具体的なモデルとワークフローとともに解説する。

---

## ⚙️ なぜM5 Maxなのか：Apple Siliconと画像生成の現在地
...
- **ユニファイドメモリ**：最大128GB
- **SSD**：最大8TB
- **メモリバンド幅**：M4 Max比で約**1.5倍**

> **重要なのは、CPU・GPU・メモリが物理的に一つのパッケージに統合されている点だ。128GBのユニファイドメモリは、実質的に「128GBのVRAM」として機能する。**

従来のPCのように、GPUのVRAMにデータをコピーするオーバーヘッドがない。これにより、**FLUX.2 dev（32Bパラメータ、BF16で約64GB）** のような巨大モデルを、ノートPC一台で動かせる時代が来た。
...
![M5 Max 制作ワークフロー](/assets/rqgm83/svg-workflow.svg)
*図2: M5 Max 制作ワークフロー。6ステップでラフから最終出力まで完結する。*

### 各層の役割

| 層 | アプリ | 役割 | 担当処理 |
|---|-------|------|---------|
| 🥇 第一層 | **Draw Things** | 高速レンダリング核（日常の9割） | ラフ生成・高品質レンダリング・LoRA学習・インペイント |
| 🥈 第二層 | **ComfyUI** | ワークフロー自動化層 | ノード設計・バッチ生成・ControlNet・エージェント連携 |
| 🥉 第三層 | **MFLUX** | 最新モデル実験層 | 最先端モデル・CLI・API・研究用途 |

それぞれの役割を見ていこう。

---

## 💨 第一層：Draw Things—高速レンダリングの中心

**Draw Things**は、macOS/iOS向けにApple Siliconへ**ネイティブ最適化**された画像生成アプリケーションだ。

M1世代から開発が続いており、M5 MaxではM4 Max比で**一部処理が3.3倍以上高速化**したとの公式測定もある。

### Draw Thingsで担当する処理

- ✅ **高速なラフ生成**（FLUX.2 klein 4B）— 約2秒
- ✅ **高品質レンダリング**（FLUX.2 dev 32B）— 約30秒
- ✅ インペイント・アウトペイント
...
**設定の初期値：**
**Universal Weights Cache**は標準で物理メモリの約半分（64GB）を使用する設計。128GB機では64GBをキャッシュに割り当て、残り64GBをmacOS・ブラウザ・レタッチソフト用に残すのが妥当だ。

---

## 🔗 第二層：ComfyUI＋Draw Thingsブリッジ—ワークフロー自動化
...
**ポイントは、ComfyUIのPyTorch MPSバックエンドを直接レンダラーに使わないことだ。**
...
**MFLUX**は、Appleの**MLX**（機械学習フレームワーク）を直接利用する画像生成基盤だ。
...
2026年6月に公開された**Krea 2 Turbo**や**Ideogram 4**のような最先端モデルも、MFLUXならMac上で動作させられる。
...
> **Draw ThingsよりGUIや統合機能は弱いが、研究用バックエンドとして優秀だ。**
...
![M5 Max 128GB モデル比較マップ](/assets/rqgm83/svg-model-comparison.svg)
*図3: M5 Max 128GBにおける各画像生成モデルのメモリ使用量・速度・用途・実行環境の比較。7モデルを用途別に最適配置できる。*
...
| 🖼️ アニメ・イラスト系 | **Anima Preview 3** | Draw Things | ~15GB |
...
- **128GB機なら実用的に常駐させられる**数少ないモデル
- ⚠️ ライセンスに制限あり。商用利用は条件確認が必要
...
> **率直な結論：M5 Max 128GBでは「巨大モデルを無理に動かすこと」より、Draw Thingsの高速ネイティブ実装を中心に複数モデルを適材適所で切り替える方が強い。**

MFLUXを追加することで、Draw Thingsが未対応の最新モデルも補完できる。
...
2025年、M4 Maxで64GBユニファイドメモリが標準になり、「VRAM 64GB相当」がノートPCで実現した。
